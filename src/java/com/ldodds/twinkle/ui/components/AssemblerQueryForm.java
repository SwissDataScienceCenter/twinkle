...
public class AssemblerQueryForm extends QueryForm
{
    ...
    private Model getModelToQuery() { 
    	try {
			...
			throw new ResultSetException( "No assembler implementation", nie);
		} catch (PropertyRequiredException pre) {
			throw new ResultSetException( "Resource incorrectly configured", pre);
		} catch (CannotConstructException e) {
			throw new ResultSetException( "Cannot construct Model", e);
		} catch (AssemblerException e) {
			throw new ResultSetException( "Unable to construct Model", e);
		}    	
    }	
}
